...
int AN;
int tt3,tt4,tt5;
float aan,ban,can,dan;// ËΩâÊèõËÆäÊï∏
int r=0,rd=0,ri=0;
char rr='9';
...
unsigned int analogRead(char channel)
{
    AUXR&=0x8F;                                 //3]ADRJ=0
    channel &= 0x07;                            
    ADCTL = 0x88|channel;                       //?}clAa°¶?  //-i?¢X ADCTL = 0x88|channel;
    while(!(ADCTL & 0x10));                    //¢X_¢XEAa°¶?-P°¬a                 
...
void sample_inputs()  {
  ...
  gangleratedeg2 = (float) ((steersum/7) - s) * 2.44;  //divide by 0.41 as for low resolution balance gyro i.e. multiply by 2.44
  	...
  	//steer
		...
		if(issteer)SteerValue=512;	                            //¬‡≈s §‚ß‚±M•Œ
		else {
...
/*else if(r<750)
{
 ...
 /*void sendduty(char left,char power){          //≤ƒ§T•Nµ{¶°
...
main()
	...
	void uart(void) interrupt 4            //±±®ÓSEGWAY   ¶Í¶C§§¬_
{
	if(TI){
		TI=0;
	}
...
t2clk=~t2clk;	
 ...
 //XXXXXXXXXXXXXXX end of softstart code XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
	...
	void PWM(unsigned char buf)     //øÈ•XPWM
{
